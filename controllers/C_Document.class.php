...
class C_Document extends Controller {
    ...
    function __construct($template_mod = "general") {
        ...
        $this->form_action = $GLOBALS['webroot'] . "/controller.php?" . $_SERVER['QUERY_STRING']; //$this->assign("CURRENT_ACTION", $GLOBALS['webroot']."/controller.php?" . "document&");
        ...
        if (count($_FILES['file']['name']) > 0) {
            ...
            foreach ($_FILES['file']['name'] as $key => $value) {
                ...
                if ($_FILES['file']['error'][$key] > 0 || empty($fname) || $_FILES['file']['size'][$key] == 0) {
                    ...
                    $d = new Document();
                    $rc = $d->createDocument($patient_id, $category_id, $fname, $_FILES['file']['type'][$key], $filetext, empty($_GET['higher_level_path']) ? '' : $_GET['higher_level_path'], empty($_POST['path_depth']) ? 1 : $_POST['path_depth'], $non_HTTP_owner);
                    if ($rc) {
                        $error .= $rc . "\n";
                    } else {
