...
/*
 * This page is used to setup https access to LibreEHR with client certificate authentication.
 * If enabled, the browser must connect to LibreEHR using a client SSL certificate that is
 * generated by LibreEHR.  This page is used to create the Certificate Authority and
 ...
 * A temporary certificate will be written to /tmp/libreehr_client_cert.p12.
...
function create_client_cert() {
    ...
    if (!$GLOBALS['is_client_ssl_enabled']) {
        $error_msg .= xl('Error, User Certificate Authentication is not enabled in LibreEHR', 'e');
        return;
    }
    ...
    $filename = $GLOBALS['temporary_files_dir'] . "/libreehr_client_cert.p12";
...
function create_and_download_certificates()
{
    ...
    $arr = create_csr("LibreEHR CA for " . $commonName, $emailAddress, $countryName, $stateOrProvinceName,$localityName, $organizationName, $organizationalUnitName);
...
<html>
  ...
  <ul>
    ...
    <li><?php xl('Configure Apache and LibreEHR to use Client side SSL certificates.', 'e'); ?>
  ...
  <div class="borderbox">
    ...
    <?php
      if (!$GLOBALS['is_client_ssl_enabled'] ||
           $GLOBALS['certificate_authority_crt'] == "") {
        echo "<font class='redtext'>" . xl('LibreEHR must be configured to use certificates before it can create client certificates.', 'e') . "</font><br>";
     }
    ?>
